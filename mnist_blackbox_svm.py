# -*- coding: utf-8 -*-
"""
Created on Wed Apr 19 18:04:36 2017
...
clf = SVC(C=10000, cache_size=1000)#, max_iter=200 )kernel=str('linear')
...
def train_substitute(sess, x, y, bbox_preds, X_sub, Y_sub):
    ...
    for rho in xrange(FLAGS.data_aug):
        ...
        if rho < FLAGS.data_aug - 1:
            ...
            #                      args=eval_params)[0]
            bbox_val=clf.predict(np.reshape(X_sub_prev,(X_sub_prev.shape[0],28*28)))
            # Note here that we take the argmax because the adversary
            # only has access to the label (not the probabilities) output
            # by the black-box model
            Y_sub[int(len(X_sub)/2):] = bbox_val#np.argmax(bbox_val, axis=1)
...
def main(argv=None):
    ...
    clf.fit( np.reshape(X_train,(X_train.shape[0],28*28)), np.argmax(Y_train, axis=1))
    bbox_preds=clf.predict(np.reshape(X_test,(X_test.shape[0],28*28)))
    ...
    eval_params = {'batch_size': FLAGS.batch_size}
    X_test_adv, = batch_eval(sess, [x], [adv_x], [X_test], args=eval_params) # return numpy
